...
begin 
	...
	data_tbl as (
		select 
					ada.oper_date as on_date, 
					ada.acc_rk as account_rk, 
					ada_cr.day_total as credit_amount,
					ada_db.day_total as debet_amount
		from
			acc_day_amount ada
		left join acc_day_amount ada_cr on
			ada_cr.acc_rk = ada.acc_rk
			and ada_cr.cr_db = 'cr'
		left join acc_day_amount ada_db on
			ada_db.acc_rk = ada.acc_rk
			and ada_db.cr_db = 'db'
			)
	select distinct
		dtbl.on_date,
	...
	from
		data_tbl dtbl
	left join ds.md_account_d mad on
		mad.account_rk = dtbl.account_rk
...
select distinct on_date from dm.dm_account_turnover_f datf 
order by on_date
